function setUsername() {
    ...
    fetch(`http://localhost:3000/api/set-username/${unique}/${token}/${newUsername}`).then(val => val.json()).then(val => {
        if(val.code=="0") {
...
function setEmail() {
    ...
    let unique = getCookie("unique")
    let token = getCookie("token")
    fetch(`http://localhost:3000/api/set-email/${unique}/${token}/${email}`).then(val => val.json()).then(val => {
        if(val.code=="0") {
...
function setBio() {
    ...
    let unique = getCookie("unique")
    let token = getCookie("token")
    fetch(`http://localhost:3000/api/set-bio/${unique}/${token}/${bio}`).then(val => val.json()).then(val => {
        if(val.code=="0") {
...
function changePassword() {
    ...
    let unique = getCookie("unique")
    let token = getCookie("token")
    fetch(`http://localhost:3000/api/set-password/${unique}/${token}/${oldPassword}/${newPassword}`).then(val => val.json()).then(val => {
        console.log(val)
        if(val.code == "0") {
